...
is.model.var <- function(varname, model.list) {
	...
	return(which(mods.with.var==TRUE))
}
...
initMELC <- function(basefiledir, modelfiledir, propensityfiledir, catToCont.modelfiledir, num.iterations) {
	
	cat("Initialising MELC\n")
	
	descriptions_dataframe <- read_file(basefiledir, "MELC data dictionary.csv")
	codings_dataframe <- descriptions_dataframe
	dict.MELC <<- Dictionary$new(descriptions_dataframe, codings_dataframe)
	...
	childsets <<- createChildSets(children, dict.MELC$codings)
	...
	#load stored runs
	#load("./Base/AllBaseRuns.rData", envir = .GlobalEnv) #(cont.outcomes.all.runs)
	
	cat("MELC initialised\n")
}
...
dirs$models <- paste(dirs$root,"models/",sep="")
dirs$catToContModels <- paste(dirs$root, "models_CatToCont/", sep="")
dirs$PropensityModels <- paste(dirs$root, "models_Propensities/", sep="")
dirs$propensityFiles <- paste(dirs$root,"propensityFiles/",sep="")
